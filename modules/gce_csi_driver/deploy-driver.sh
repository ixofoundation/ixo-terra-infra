...
function check_service_account()
{
	...
	readonly IAM_NAME=$(awk -F '[:"]' '/client_email/ {print $5}' "${GCE_PD_SA_DIR}/cloud-sa.json")
  readonly PROJECT=$(awk -F '[:"]' '/project_id/ {print $5}' "${GCE_PD_SA_DIR}/cloud-sa.json")
	...
	echo "BINDED ROLES ${GOTTEN_BIND_ROLES}"
	MISSING_ROLES=false
